...
<script type="text/javascript">
	jQuery(document).ready(function(){
		jQuery('.add_panel_image').click(function(e){
			e.preventDefault();
			var level = jQuery(this).data('buttonlevel');
			var counter = jQuery(this).data('buttoncounter');
			if(level == <?php echo $level;?> && counter==<?php echo $counter;?>)
			{
...
<link rel="stylesheet" type="text/css" href="<?php plugins_url('Rev_Custom_Slider/css/style.css');?>">
	<table class="color-box">
		<tr>
			<th scope="row"><h3>Color Options <?php echo $counter;?></h3></th>
		...
		<tr>
			...
			<td><input name="color_name[<?php echo $level;?>][<?php echo $counter;?>][]" type="text" id="color_name"  class="regular-text"></td>
		</tr>
		<tr>
			<th scope="row"><label for="color_image">Color Image</label></th>
			<td><input name="color_image[<?php echo $level;?>][<?php echo $counter;?>][]" type="file" id="color_image"  class="regular-text"></td>
		</tr>
		<tr>
			<th scope="row"><label for="panel_image">Panels Image</label></th>
			<td class="panel_image_<?php echo $level;?>_<?php echo $counter;?>">
				<input name="panel_image[<?php echo $level;?>][<?php echo $counter;?>][]" type="file" id="panel_image"  class="regular-text"><br>
			</td>
		</tr>
		<tr>
			<th scope="row"></th>
			<td>
				<div class="add_panel_image div-width" data-buttonlevel="<?php echo $level;?>" data-buttoncounter="<?php echo $counter;?>">
