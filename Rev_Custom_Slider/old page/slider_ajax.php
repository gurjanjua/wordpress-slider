...
$HostName = 'http://'.$_SERVER['HTTP_HOST'].'/wordpress';
$level=$_REQUEST['level'];
$id=$_REQUEST['id'];
$page_id=$_REQUEST['page_id'];
?>
<script src="<?php echo plugins_url('Slider/js/swiper.jquery.js');?>"></script>
<script type= "text/javascript">
	...
	jQuery(document).ready(function(){
		...
		function switchAjax(level,counter,id){
			jQuery.ajax({
				type:"GET",
				url:"http://<?php echo $_SERVER['HTTP_HOST'];?>/wordpress/wp-content/plugins/Slider/slider_panel_image.php",
		...
		<div>
			<div class="div-left">	
				<a href="<?php echo $HostName;?>/?page_id=<?php echo $page_id;?>&tab=show&id=<?php echo $prev_id;?>">
					<img src="<?php echo plugins_url('Slider/icon/prev.png');?>" alt="previous decor" id="previous decor" class="next-prev left"/>
			...
			<div class="div-left center">
				<a href="<?php echo $HostName;?>/?page_id=<?php echo $page_id;?>">
					<span class="text">Back to Catalogue</span>
				</a>
			</div>
			<div class="div-left">
				<a href="<?php echo $HostName;?>/?page_id=<?php echo $page_id;?>&tab=show&id=<?php echo $next_id;?>">
					<img src="<?php echo plugins_url('Slider/icon/next.png');?>" alt="next decor" id="next decor" class="next-prev right"/>
			...
			for($i=$level;$i<=$level;$i++)
			{
				...
				for($j=1;$j<=$length;$j++)
				{
					...
					<div title="<?php echo $color_name[$i][$j][0];?>">
						<img src="<?php echo plugins_url('Slider/color_image/'.$images)?>" class="color-image" alt="<?php echo $images;?>" level="<?php echo $i;?>" counter="<?php echo $j;?>" id="<?php echo $id;?>"/>
					</div>
					<?php 
				}
			}
